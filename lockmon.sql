...
SET PAGESIZE 2000 LINESIZE 250 DEFINE ON VERIFY OFF FEEDBACK OFF TERMOUT OFF
...
(
  SELECT * 
  FROM ALL_LOCKS
  WHERE TYPE NOT IN ( 'XR', 'RT', 'TS', 'MR', 'CF', 'PW', 'RS', 'TO', 'PS', 'KD','CO','RD','DM','KT' )
),
LOCKS AS
(
  SELECT
...
(
  SELECT /*+ ALL_ROWS */
    ( SELECT U.NAME || '.' || O.NAME FROM SYS.OBJ$ O, SYS.USER$ U WHERE OBJ# = V.OBJECT_ID AND U.USER# = O.OWNER# ) OBJETO
