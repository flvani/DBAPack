...
COL SQL_HASH     FORMAT            A13 HEAD "Sql ID"
...
BEGIN
  ...
  ELSIF SUBSTR( :B1, 1, 1 ) = '#' THEN
    :V1 := 'AND S.PADDR IN ( SELECT P.ADDR FROM &p_gv.$PROCESS P WHERE P.SPID='||SUBSTR( :B1, 2, LENGTH(:B1))||')';
  ELSIF SUBSTR( :B1, 1, 1 ) = '@' THEN
...
WITH TOP_SESSION AS
( SELECT &p_s_inst. INST, S.SID, S.SERIAL# SERIAL, S.STATUS, S.SERVER, P.SPID
       ...
       , S.PROGRAM PROGRAMA, S.PADDR, S.SQL_ID
...
TOPN AS 
(  -- LIMITA O NUMERO DE LINHAS DO RETORNO
   SELECT /*+ NO_MERGE(S) */ S.*, LPAD(ROWNUM, 3, ' ' ) || ':' RANK
   FROM TOP_SESSION S
   WHERE ROWNUM <= &N_TOPS.
),
SESSOES AS
( -- ACRESCENTA ALGUNS DETALHES
  SELECT /*+ NO_MERGE(T) */ S.*, SUBSTR( W.EVENT, 1, 26 ) WAITS
  FROM TOPN S
  LEFT JOIN &P_GV.$SESSION_WAIT W ON (W.SID=S.SID AND &p_w_inst. = S.INST)
  ORDER BY S.RANK
)
SELECT 
   SE.RANK
  ...
  ,SE.MACHINE
  ,SE.PROGRAMA
  ,(SELECT ST1.SQL_TEXT FROM &p_gv.$SQLTEXT ST1 WHERE SE.SQL_ID = ST1.SQL_ID AND ST1.PIECE = 0 AND ROWNUM = 1) ||
   (SELECT ST1.SQL_TEXT FROM &p_gv.$SQLTEXT ST1 WHERE SE.SQL_ID = ST1.SQL_ID AND ST1.PIECE = 1 AND ROWNUM = 1) SQL_TEXT
FROM SESSOES SE
/
