...
BEGIN
  ...
  ELSIF SUBSTR( :B1, 1, 1 ) = '#' THEN
    :V1 := 'AND (S.INST_ID,S.PADDR) IN ( SELECT INST_ID,P.ADDR FROM &p_gv.$PROCESS P WHERE P.SPID='||SUBSTR( :B1, 2, LENGTH(:B1))||')';
  ELSIF SUBSTR( :B1, 1, 1 ) = '@' THEN
...
(
  ...
  &CLS_WHERE_ACTIVE.
  &CLS_NO_BACKGROUND
  ORDER BY SS.VALUE DESC FETCH FIRST &N_TOPS. ROWS ONLY
), RANKED_SESSIONS AS
(
  SELECT 
    LPAD(ROWNUM, 3, ' ' ) || ':' RANK
...
SELECT 
  ...
  ,SE.MACHINE
  ,SE.PROGRAMA
  ,ST1.SQL_TEXT || ST2.SQL_TEXT SQL_TEXT
