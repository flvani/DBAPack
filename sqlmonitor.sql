...
FROM (
  SELECT /*+ NO_XML_QUERY_REWRITE */ x1.username, x1.sql_exec_start, x1.sql_id, x1.plan_hash, trunc(x1.elapsed_time/1000000,2) elap_sec, substr(x1.sql_text, 1, 50) sql_text
  ...
  and  x1.elapsed_time/1000000 > 30
  --and  x1.sql_id = 'a9yr4qy05y1yc'
  and to_date(x1.sql_exec_start, 'MM/DD/YYYY HH24:MI:SS') between trunc(sysdate)-1 and trunc(sysdate)
  order by 5 desc 
