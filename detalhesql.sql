...
COL PROGRAM_ID NEW_VALUE P_PROGRAM_ID NOPRINT
COL SOURCE FORMAT A70
...
select object_type || ': ' || owner ||'.'|| object_name SOURCE
from dba_objects  where object_id='&P_PROGRAM_ID.'
/
...
DECLARE
  ...
  TYPE STRING_LIST IS TABLE OF VARCHAR2(200);
  V_TXT   VARCHAR2(32000) := '';
  V_LINHAS_SQL STRING_LIST := STRING_LIST();
  ...
  FUNCTION BREAK_LINE( V_TXT VARCHAR2, LINHAS IN OUT STRING_LIST) RETURN VARCHAR2
  IS
    NLEN    PLS_INTEGER ;
    IDX     PLS_INTEGER := 0;
    NPOS    PLS_INTEGER := 0;
  ...
  BEGIN
    ...
    WHILE IDX < NLEN LOOP
      ...
      END IF;
      LINHAS.EXTEND();
      LINHAS(LINHAS.COUNT) := REPLACE(REPLACE(SUBSTR( V_TXT, IDX+1, NPOS ), CHR(10), '' ), CHR(13), '' );
      IDX := IDX + NPOS;
    END LOOP;
  END;
...
BEGIN
  ...
  LOOP
    ...
    IF LENGTH(V_TXT) > 31900 THEN
      V_TXT := BREAK_LINE(V_TXT, V_LINHAS_SQL);
    END IF;
  END LOOP;

  V_TXT := BREAK_LINE(V_TXT, V_LINHAS_SQL);
  V_LINHAS_SQL.EXTEND();
  V_LINHAS_SQL(V_LINHAS_SQL.COUNT) := REPLACE(REPLACE( V_TXT , CHR(10), '' ), CHR(13), '' );

  IF V_LINHAS_SQL.COUNT > 0 THEN
    FOR IDX IN V_LINHAS_SQL.FIRST .. V_LINHAS_SQL.LAST
    LOOP
      DBMS_OUTPUT.PUT_LINE(V_LINHAS_SQL(IDX));
    END LOOP;
